  ...
  for player_json in select * from jsonb_array_elements(room.players) loop
    players_json := players_json || jsonb_build_array(
      jsonb_build_object(
        ...
        'avatar', coalesce(player_json->>'avatar', case when coalesce((player_json->>'bot')::boolean, false) then 'bot' else 'img1' end),
...
create or replace function ww_create_room(p_name text, p_avatar text default 'img1')
returns jsonb
language plpgsql
security definer
