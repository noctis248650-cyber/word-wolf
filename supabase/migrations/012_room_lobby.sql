...
drop function if exists ww_create_room(text, text);
...
begin
  ...
  values (
    room_code,
    player_id,
    jsonb_build_object('discussionSeconds', 180, 'wolfCount', 1, 'title', clean_title, 'maxPlayers', clean_max_players),
...
grant execute on function ww_create_room(text, text, text, integer) to anon;
